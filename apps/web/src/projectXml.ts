...
function blockForStep(step: ProgramStep): string {
  switch (step.kind) {
    case "digital-write":
      return `<block type="abl_led_write">${field("COMPONENT", step.componentId)}${field("STATE", step.value === "LOW" || step.value === false ? "LOW" : "HIGH")}</block>`;
    case "delay":
    ...
    case "read-analog-serial":
      return `<block type="abl_analog_serial">${field("SENSOR", step.componentId)}</block>`;
    case "read-digital-serial":
      return `<block type="abl_digital_serial">${field("SENSOR", step.componentId)}</block>`;
    case "ir-read-serial":
